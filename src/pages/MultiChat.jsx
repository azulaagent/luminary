...
export default function MultiChat() {
  ...
  const generateNext = useCallback(async () => {
    ...
    } catch (e) { setMessages(prev => [...prev.slice(0, -1), { role: 'system', content: `Error: ${e.message}`, agentName: 'System' }]) }
    setIsRunning(false)
    ...
    <div className="flex-1 flex flex-col">
      ...
      <div className="flex items-center gap-4 px-6 py-3.5 border-b border-white/[0.04] shrink-0 bg-bg/60 backdrop-blur-xl">
        ...
        <div className="flex items-center gap-2">
          {participantAgents.map(a => (
            <motion.button key={a.id} whileHover={{ scale: 1.08 }} onClick={() => toggleParticipant(a.id)} className="ring-2 ring-accent/30 rounded-full p-0.5 hover:ring-accent/60 transition-all">
              <ProceduralAvatar agent={a} size={32} />
            </motion.button>
          ))}
          {participantAgents.length < agents.length && (
            <div className="relative group">
              <button className="w-9 h-9 rounded-full border border-dashed border-white/[0.08] flex items-center justify-center text-text-dim hover:text-text-muted hover:border-white/[0.15] transition-colors"><Plus size={12} /></button>
        ...
        <div className="flex gap-2 ml-3">
          <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} onClick={() => { setAutoMode(!autoMode); if (!autoMode && messages.length === 0) generateNext() }} className={`flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-[11px] font-medium transition-all ${autoMode ? 'bg-accent/15 text-accent-bright border border-accent/20 glow-sm' : 'bg-white/[0.03] border border-white/[0.06] text-text-muted hover:text-text hover:border-white/[0.1]'}`}>
            {autoMode ? <Pause size={11} /> : <Play size={11} />} {autoMode ? 'Auto' : 'Manual'}
          </motion.button>
          <button onClick={() => setMessages([])} className="p-2 rounded-xl bg-white/[0.03] border border-white/[0.06] text-text-dim hover:text-text-muted transition-colors"><RotateCcw size={13} /></button>
        </div>
      </div>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-32">
              <div className="flex justify-center gap-3 mb-5">
                {participantAgents.map((a, i) => (
                  <motion.div key={a.id} initial={{ opacity: 0, scale: 0 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.15, type: 'spring' }}>
              ...
              <p className="text-sm text-text-muted">{participantAgents.length < 2 ? 'Add at least 2 agents to start' : 'Press Play or type a message to begin'}</p>
            </motion.div>
          )}
          {messages.map((msg, i) => {
            ...
            return (
              <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className={`flex gap-3 ${isUser ? 'justify-end' : ''}`}>
                {!isUser && msg.agent && <div className="shrink-0 mt-1"><ProceduralAvatar agent={msg.agent} size={24} /></div>}
      ...
      <div className="px-6 py-4 border-t border-white/[0.04] shrink-0 bg-bg/60 backdrop-blur-xl">
        <div className="flex gap-3 items-end max-w-3xl mx-auto">
          <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') sendAsUser() }} placeholder="Join the conversation..." className="flex-1 input-field py-3" />
          <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={sendAsUser} disabled={isRunning || !input.trim()} className="p-3 rounded-xl bg-accent hover:bg-accent/80 disabled:opacity-30 transition-all glow-sm"><Send size={16} /></motion.button>
        </div>
      </div>
    </div>
  )
